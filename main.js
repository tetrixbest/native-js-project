...
const inpAdd = document.querySelector('#btnAdd');
const sectionBooks = document.querySelector("#flush-collapseOne");
const section = document.querySelector("#section")

// 
let searchValue = ""
// переменные для кнопок пагинаций
const LIMIT = 6;
const prevBtn = document.querySelector('#prevBtn');
const nextBtn = document.querySelector('#nextBtn');

// переменные для пагинаций
let currentPage = 1;
let countPage = 1;

btnAdd.addEventListener('click', () => {
    if(
        // proverks na zapolnenie
        !inpName.value.trim() ||
        !inpDesc.value.trim() ||
        !inpPrice.value.trim() ||
        !inpImage.value.trim() 
...
document.addEventListener('click', async ({ target: { classList, id } }) => {
    ...
    if(classes.includes('btnEdit')) {
        ...
        const { title, description, image, price, id: productId } = await res.json();
